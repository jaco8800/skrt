...
module.exports.run = async (bot, message, args) => {
  ...
  if (!bUser) return message.channel.send("User does not exist.");

  const bReason = args.join(" ").slice(22);

  if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("No permission.");
    
  if (bUser.hasPermission("BAN_MEMBERS")) return message.channel.send("You can't kick a moderator.");
    
  if (!bReason) return message.channel.send("Give reason");
